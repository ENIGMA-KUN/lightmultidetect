...
const FrequencyAnalysis: React.FC<FrequencyAnalysisProps> = ({
  ...
  useEffect(() => {
    ...
    if (data.low_freq_energy !== undefined) {
      ...
      frequencyData = Object.entries(data).filter(([key, value]) => typeof value === 'number');
    }
    
    // Find max value for scaling
    const maxValue = Math.max(...frequencyData.map(([_, value]) => value as number));
    ...
    frequencyData.forEach(([label, value], index) => {
      const barHeight = ((value as number) / maxValue) * (height - 60);
      ...
      ctx.font = '10px Arial';
      ctx.fillText((value as number).toFixed(2), x + (barWidthWithPadding / 2), y - 15);
    });
          ...
          <div className="mt-2 p-3 bg-gray-900 rounded-lg">
            <div className="flex justify-between items-center">
              ...
              <span className={`font-medium ${
                data.high_to_low_ratio > 0.5 ? 'text-red-400' : 'text-green-400'
              }`}>
